	...
	m_Box (Gtk::ORIENTATION_VERTICAL)
{
	...
	refActionGroup->add_action ("newstandard", sigc::mem_fun (*this, &ExampleWindow::on_menu_file_new_generic));
	refActionGroup->add_action ("newfoo",      sigc::mem_fun (*this, &ExampleWindow::on_menu_file_new_generic));
	refActionGroup->add_action ("newgoo",      sigc::mem_fun (*this, &ExampleWindow::on_menu_file_new_generic));

	refActionGroup->add_action ("quit",        sigc::mem_fun (*this, &ExampleWindow::on_menu_file_quit));

	refActionGroup->add_action ("copy",        sigc::mem_fun (*this, &ExampleWindow::on_menu_others));
	refActionGroup->add_action ("paste",       sigc::mem_fun (*this, &ExampleWindow::on_menu_others));
	refActionGroup->add_action ("something",   sigc::mem_fun (*this, &ExampleWindow::on_menu_others));

	//Choices menus, to demonstrate Radio items,
	//using our convenience methods for string and int radio values:
	m_refChoice      = refActionGroup->add_action_radio_string  ("choice",      sigc::mem_fun (*this, &ExampleWindow::on_menu_choices),       "a");
	m_refChoiceOther = refActionGroup->add_action_radio_integer ("choiceother", sigc::mem_fun (*this, &ExampleWindow::on_menu_choices_other), 1);
	m_refToggle      = refActionGroup->add_action_bool          ("sometoggle",  sigc::mem_fun (*this, &ExampleWindow::on_menu_toggle),        false);

	//Help menu:
	refActionGroup->add_action ("about", sigc::mem_fun (*this, &ExampleWindow::on_menu_others));
 ...
 */
void
ExampleWindow::on_menu_choices (const Glib::ustring& parameter)
{
 ...
 */
void
ExampleWindow::on_menu_choices_other (int parameter)
{
	//The radio action's state does not change automatically:
